...
int main(int argc, char *argv[]) {
    ...
    if (strcmp(arg1, "-p") == 0) {
	    ...
	    if (res == TEEC_ERROR_NOT_SUPPORTED) {
	    	fprintf(stderr, "\033[31mTEEC_InvokeCommand (optee_example_cnn -p [cnn_param]) not implemented\033[0m");
            goto exit;
        }
        ...
        res = TEEC_ERROR_NOT_SUPPORTED;
        if (res == TEEC_ERROR_NOT_SUPPORTED) {
	    	fprintf(stderr, "\033[31mTEEC_InvokeCommand (optee_example_cnn -i [img]) not implemented\033[0m");
            goto exit;
        }
    } else {
